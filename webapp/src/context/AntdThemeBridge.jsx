...
const components = {
  ...
  Dropdown: {
    // The Dropdown popup floats over the rail, so give it the `--elevated`
    // stock so it reads as a distinct panel (matching Select's treatment).
    colorBgElevated: cssVar("--elevated"),
    // Re-introduce elevation that the global boxShadow:none clears: a hairline
    // `--rule` ring (the border) plus a soft drop shadow. antd applies this to
    // the popup root, so the two layers give us border + shadow in one token.
    boxShadowSecondary:
      "0 0 0 1px rgb(var(--rule)), 0 8px 24px -8px rgb(0 0 0 / 0.35)",
  },
  Drawer: {
  ...
  Input: {
    borderRadius: 10,
    // antd draws its own focus treatment on Input/TextArea — an `activeBorderColor`
    // plus an `activeShadow` glow — which flashes over the app's Tailwind border
    // for a beat on focus. Neutralize both so the consumer's `focus:border-*`
    // class is the single source of truth for the focused outline.
